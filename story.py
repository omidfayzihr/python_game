# story.py - Main story flow with proper logic
import sys
from player import *
...
def start():
    ...
    opt = ask_choice("❓ What now?", "What do you want to do?", ["👀 Look Around"])
    if opt == "👀 Look Around":
        start_area_intro()
    else:
        start()
...
def start_area_intro():
    """First time entering the temple"""
    _hud()
...
def golden_door_check():
    """Check golden door - requires both keys"""
    _hud()

    if has_both_keys():
        # Player has both keys - can open
        alert("🔑 Keys Found!",
...
def statue_room():
    ...
    opt = ask_choice("🗿 Statue", "What do you want to do?", ["✋ Take Key", "⬅️ Back"])
...
def exit_door():
    ...
    opt = ask_choice("🚪 Exit", "What do you want to do?", ["🚶 Go Outside", "⬅️ Back"])
...
def crossroads():
    """Crossroads - dynamic options based on progress"""
    _hud()
    ...
    # If both are done, offer to go back
...
def return_to_temple():
    """Return to temple after completing both paths"""
    _hud()
    alert("🏛️ Return",
...
def open_golden_door():
    """Open the golden door - final sequence"""
    _hud()
